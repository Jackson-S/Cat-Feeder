...
def get_feed_from_db():
    db = sqlite3.connect(database_name)
    query = "SELECT datetime(Time, 'localtime') FROM FeedTimes ORDER BY Time DESC"
...
def get_last_feed_from_db():
    db = sqlite3.connect(database_name)
    query = "SELECT datetime(Time, 'localtime') FROM FeedTimes ORDER BY Time DESC LIMIT 1"
...
def get_check_from_db():
    db = sqlite3.connect(database_name)
    query = "SELECT datetime(Time, 'localtime') FROM CheckTimes ORDER BY Time DESC"
